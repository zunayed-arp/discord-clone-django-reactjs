...
def ForbiddenUsers(value):
    forbidden_users = ['admin','css','js','authentication','login','logout','administrator','root','email','user','join','sql','static','python','delete']
    
    
    if value.lower() in forbidden_users:
        raise ValidationError('Invalid name for user, this is a reserved woord.')
def InvalidUser(value):
    if '@' in value or '+' in value or '-' in value:
        raise ValidationError('This is an Invalid user, Do not Use these characterts: @, -, +, "')

def UniqueEmail(value):
    if User.objects.filter(email__iexact = value).exists():
        raise ValidationError('User with this emaill already exists.')

def UniqueUser(value):
    if User.objects.filter(username__iexact=value).exists():
        raise ValidationError('User with this username alraedy exists.')
...
class SignupForm(forms.ModelForm):
    ...
    password = forms.CharField(widget=forms.PasswordInput(),max_length=30,required=True,label="Confirm you password.")
    ...
    def clean(self):
        ...
        if password != confirm_password:
            self._erros['password']=self.error_class(['Passwrods do not match.Try again'])
...
class ChangePasswordForm(forms.ModelForm):
    ...
    confrim_password = forms.CharField(widget=forms.PasswordInput(attrs={'class':'input is-medium'}),label="Confirm new password", required=True)
    ...
    def clean(self):
        ...
        if not user.check_password(old_password):
            self._erros['old_password'] = self.error_class(['old password do not match.'])
...
class EditProfileForm(forms.ModelForm):
    ...
    picutre = forms.ImageField(required=False)
